...
describe Board do
    ...
    #     board_3 = Board.new
    #     board_3.grid[-3][4] = board_3.white_rook
    #     board_3.grid[-5][4] = board_3.black_pawn
    #     board_3.update_board(board_3.white_rook, [-3, 4], -5, 4)
    #     it 'expect place where previously was black pawn to now be occpied by a white rook' do
    #         expect(board_3.grid[-5][4] == board_3.white_rook).to be true
    #     end
    # end

    context "check?" do
        game = Game.new
        it 'returns check when white king is in check' do
            game.board.grid[-3][4] = WHITE_KING
            game.board.grid[-5][4] = BLACK_K_ROOK
            expect(game.board.check?).to be true
        end
    end
